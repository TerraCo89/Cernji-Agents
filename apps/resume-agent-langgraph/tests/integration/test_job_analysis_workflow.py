...
def test_job_analysis_workflow_state_schema():
    """
    Test JobAnalysisState schema validation.

    Validation:
    - Required fields can be provided
    ...
    Note: JobAnalysisState is a TypedDict, not a Pydantic model,
    so validation is done at runtime by type checkers, not at instantiation.
    """
    ...
    # Test state with analysis populated
    state_with_analysis: JobAnalysisState = {
        "job_url": "https://example.com/job",
        "job_content": "Job posting content",
        "job_analysis": {
            ...
            "candidate_profile": "Experienced Python developer",
            "raw_description": "Job posting content",
        },
        "cached": True,
        "errors": [],
        "duration_ms": 1500.0,
    ...
    assert state_with_analysis["job_analysis"] is not None
    assert state_with_analysis["cached"] is True
